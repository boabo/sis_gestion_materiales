...
<script>
    Phx.vista.Solicitud=Ext.extend(Phx.gridInterfaz,{
        ...
        constructor:function(config){
            ...
            this.bbar.el.dom.style.background='#A7D6F0';
    				this.tbar.el.dom.style.background='#A7D6F0';
    				this.grid.body.dom.firstChild.firstChild.firstChild.firstChild.style.background='#C8E9FE';

    				this.grid.body.dom.firstChild.firstChild.lastChild.style.background='#D0F5FF';
        ...
        Atributos:[
            ...
            /*Aumentando para poner el tiempo de entraga (9/11/2020)*/
            {
                config:{
                    ...
                    allowBlank: true,
                    width: 200,
                    qtip: 'Poner los dias de entrega de la propuesta<br><b>Este campo se mostrará en el reporte de la invitación</b>',
                    gwidth: 200,
                    maxLength:10000
                },
                type:'NumberField',
        ...
        sigEstado: function(){
            ...
            var tipo_tramite = tramite.substring(0,2);
                console.log("aqui llega el dato para saber el estado",rec.data);
                if(rec.data.estado == 'cotizacion' || rec.data.estado == 'compra' /*|| (rec.data.estado == 'revision' && tipo_tramite != 'GR')*/){
                    if(rec.data.lista_correos=='' || rec.data.estado == 'compra' || rec.data.tipo_solicitud == '' || (rec.data.estado == 'cotizacion' && (rec.data.tiempo_entrega == '' || rec.data.tiempo_entrega == null))) {
                        this.onButtonEdit();
        ...
        confirmarEstado:function () {
            ...
            var tramite = rec.data['nro_tramite'];
            var tipo_tramite = tramite.substring(0,2);
            if ((tipo_tramite == 'GM' && (rec.data.nro_po == '' || rec.data.nro_po == null) && rec.data.origen_solicitud == 'control_mantenimiento' /*|| tipo_tramite == 'GO' || tipo_tramite == 'GA'*/) && rec.data.estado == 'compra') {
